...
class JenkinsJob {
  constructor(jobName="Performance Tests", projectName="Genresmanagement", url="http://dev-test-jenkins-aws-migration01.aws.tfly-internal.com:8080/job/Prod01/job/") {
    ...
    this.projectUrl = `${url}/${this.jobName}/job/${this.projectName}/`;
    this.request = fetch(`${this.projectUrl}/api/json\?pretty\=true`).then((request) => request.json());
  }
  ...
  async getBuildDuration(jobUrl) {
    ...
    return fetch(apiUrl).then((request) => request.json()).then(({ duration }) => duration).catch(console.log);
  }
